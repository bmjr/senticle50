...
class Command(BaseCommand):
    ...
    def new_model(self):
        ...
        class_amount = max(len(positive_tokens), len(negative_tokens))
        print(class_amount)
        ...
        sample_list = []
        sample_list.extend(positive_tokens)
        sample_list.extend(negative_tokens)
        ...
        x_resampled, y_resampled = RandomOverSampler(random_state=0).fit_sample(x_2d, y)
        ...
        #print([item for item, count in collections.Counter(x_resampled.flatten()).items() if count > 1])


        positive_tokens_resampled = x_resampled[class_amount:]
        negative_tokens_resampled = x_resampled[:class_amount]
        ...
        for i in range(0, class_amount):
            training_set.append((positive_tokens_resampled[i], y_resampled[i]))
            training_set.append((negative_tokens_resampled[i], y_resampled[class_amount + i]))
            print(i)
        #
        ...
        # Save the tokenizer
        with open('sentiment_tokenizer.pickle',
                  'wb') as handle:
        ...
        brexit_model.fit(train_x, np.asarray(list(train_y)),
                         validation_data=(
                             test_x, np.asarray(list(test_y))), epochs=6,
                         batch_size=128,
                         verbose=2,
                         callbacks=[
        ...
        with open("sentiment_keras_model.json",
                  "w") as json_file:
            json_file.write(model_json)
        brexit_model.save_weights(
            "sentiment_keras_weights.h5")
        print("Model Saved")
